...
| 1c — HN phrase match | ✅ new 2026-09-26. Algolia matches loosely: ~1% of hits contained the query phrase, and 30 hits covered ~2h of 48h. Now 1 request/query fetches the whole window (`hits_per_query: 1000`) and `phrase_match: true` keeps only real phrase hits. Result: HN ~187 noise posts/day → ~1. Old HN posts: 0 real problems in triage. Queries reworded same day from measured 48h exact-hit counts (10 short phrases, ~83 hits) + all Ask HN stories (`ask_hn: true`, ~30/day). HN now fills its 9-post shortlist cap with no other posts displaced. |
| 1d — RSS coverage | ✅ new 2026-09-26. Default forum feeds list latest replies, spanning minutes on busy forums. Switched to new-thread feeds: XenForo `?order=post_date`, phpBB `?mode=topics`, Invision paged (`page_url`/`max_pages`); `also_urls` merges a reply feed (trade2win). Span of the 48h window: elitetrader 13.7→45h, bogleheads 0.2→18h, garagejournal 1.9→45h, watchuseek 0.6→9.7h, eurobricks 2.7→18.7h, hardwarezone 0.1→2.7h. styleforum unchanged (post_date feed broken), purseblog 403. RSS posts 181→257; shortlist share still ~2 — the keyword filter, not coverage, is now the limit. |
| 1e — owner/seller sources | ✅ new 2026-09-26. Probed 45 seller/shop-owner communities; added dealerrefresh (car dealer ops), ninjatrader + tradovate (paying traders), bunpro (paid Japanese SRS), woocommerce (shop owners). Rejected: autogeekonline, toyark, cgccomics (hobby/sale chat), prestashop (spam), squarespace (CSS help); no usable fashion or toy seller source found. Keyword study: no industry keywords added (all lift was noise). |
...
| 2b — always_channels | ✅ new 2026-09-26. Owner communities in `scoring.yml always_channels` skip the pain/money score test (0/22 dealerrefresh, 0/12 ninjatrader passed it); up to `max_always` (10), round-robin per channel, on top of `total`. |
...
| 3c — triage gate + gigs | ✅ new 2026-09-26. Clusters may only cite posts triaged `real_problem`. Posts triaged `paid_gig` (someone hiring a freelancer) get a **Gigs** section in the report + Telegram — work to take, not ideas; not scored. Both depend on the best-effort triage call: if it fails, no gating and no gigs that day. Stage 2 has a hiring pass (`hiring_regex` / `hiring_exclude` in keywords.yml) that sends up to `max_gigs` hiring posts to the AI on top of the normal shortlist, bypassing `exclude` and scoring. |
| 3d — cross-day dedupe | ✅ new 2026-09-26 (`radar/history.py`). Reads links from the last `dedupe_days` (30) of `reports/DATE.md`. An idea is dropped only if ALL its posts were reported before (logged in rejected/ as `already reported`); gigs are dropped if already shown. Discourse URLs match on topic id. No state file. |
